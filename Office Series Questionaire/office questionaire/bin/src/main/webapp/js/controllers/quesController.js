...
function quesController($http,$scope){
	...
	_refreshCustomerData();
  	function _refreshCustomerData() {
                    ...
                    $http.get('http://localhost:9595/fetchallQuestions').then(function(response) {
						...
						localStorage.setItem("questionBank",JSON.stringify(newObj));
	...
	function finaliseAnswers(){
		...
		var jsonObj = JSON.parse(localStorage.getItem("questionBank"));
	...
	function calculateLength(){
		var length = 0;
		for(var i=0;i<JSON.parse(localStorage.getItem("questionBank")).length;i++){
			if(JSON.parse(localStorage.getItem("questionBank"))[i]!== null){
				length++;
			}
		}
		return length;
